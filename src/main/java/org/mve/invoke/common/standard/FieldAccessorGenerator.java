...
public class FieldAccessorGenerator extends AccessibleObjectAccessorGenerator
{
	...
	public void generate()
	{
		...
		code.field(Opcodes.GETSTATIC, this.bytecode.name, JavaVM.CONSTANT[5], Generator.signature(AccessibleObject.class))
			...
			.method(Opcodes.INVOKESTATIC, Generator.type(Generator.class), JavaVM.CONSTANT[7], MethodType.methodType(FieldAccessor.class, org.mve.asm.attribute.code.Field.class, Object[].class).toMethodDescriptorString(), false)
			.instruction(Opcodes.ARETURN)
			.max(5, 3);
		mw.attribute(code);
