...
public abstract class DynamicBind extends Generator
{
	...
	public DynamicBind(Class<?> handle, Class<?> target)
	{
		...
		this.target = target;
		this.bytecode.set(0x34, 0x21, UUID.randomUUID().toString().toUpperCase(), CONSTANT_POOL[0], new String[]{Generator.type(handle)});
		this.bytecode.field(new FieldWriter().set(AccessFlag.PRIVATE | AccessFlag.STATIC, "0", Generator.signature(Class.class)));
	}
